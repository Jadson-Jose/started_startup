...
def sugestao(request):
    ...
    if request.method == 'GET':
        return render(request, 'sugestao.html', context= {'areas':areas})
...
def realizar_proposta(request, id):
    ...
    if total + float(percentual) > empresa.percentual_equity:
        ...
        return redirect(f'/ivestidores/ver_empresa/{id}')
    
    valuation = (100 * int(valor)) / int(percentual)
    
    if valuation < (int(empresa.valuation / 2)):
        messages.add_message(request, constants.WARNING, f'Seu valuation foi R${valuation} e deve ser de no mínimo {empresa.valuation}')
